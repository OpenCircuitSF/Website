...
| `migrations/` | `golang-migrate` up/down SQL pairs, contiguous `000001`–`000022` |
...
| 2 | SES — **the instance IAM role, then production access** | real sends; **first admin sign-in**, which mails a magic link | **mostly done 2026-08-25.** Identity `mailing.opencircuitsf.com` verified in `us-west-2`, Easy DKIM `SUCCESS`, custom MAIL FROM `bounce.mailing.…` `SUCCESS`, DMARC at `p=none` on the subdomain, configuration set + SNS topic + auto-confirmed HTTPS subscription all live, account suppression list already on. **Two blockers left:** (a) the instance has **no IAM role attached at all** — see [`docs/aws-iam-setup.md`](docs/aws-iam-setup.md), nothing sends without it; (b) the account is still sandboxed (`ProductionAccessEnabled: false`). Independent of each other; when both clear, flip `SES_SANDBOX=false` and `SEND_WORKER_ENABLED=true`. Details in [`docs/email-setup.md`](docs/email-setup.md) |
...
| **§6 Mailing List — The Core Subsystem** | 942 | `sed -n '/^## 6\. /,/^## [0-9]/p' PRD.md` |
| §6.1 Interest taxonomy | 23 | `sed -n '/^### 6\.1 /,/^#\{2,3\} [0-9]/p' PRD.md` |
| §6.2 Database schema | 335 | `sed -n '/^### 6\.2 /,/^#\{2,3\} [0-9]/p' PRD.md` |
