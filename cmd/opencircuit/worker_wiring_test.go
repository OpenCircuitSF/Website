...
func TestNewSendWorkerIfEnabled_MailerNoOp_Refuses(t *testing.T) {
	...
	w, err := newSendWorkerIfEnabled(cfg, sendStore, nil, nil, nil, nil, nil, nil, nil, nil, discardLogger())
	if err != nil {
...
func TestNewSendWorkerIfEnabled_Enabled_ConstructsWorker(t *testing.T) {
	...
	w, err := newSendWorkerIfEnabled(cfg, sendStore, audienceStore, nil, nullMailer{}, nullSettings{}, nil, nil, nil, nil, discardLogger())
	if err != nil {
		t.Fatalf("newSendWorkerIfEnabled: %v", err)
	}
	if w == nil {
