...
| `SES_CONFIGURATION_SET` | **yes**, unless `SEND_WORKER_ENABLED=false` | — | SES configuration set used for transactional/campaign sends. Listed as optional here until 2026-08-25, when the first production boot proved otherwise: `mailing.NewSESMailer` returns `cannot construct SES mailer: missing SES_CONFIGURATION_SET` and the service crash-loops without it. The named set need not exist in SES yet. Currently `opencircuit-transactional`. `#0472`: leaving this unset while also setting `SEND_WORKER_ENABLED=false` is a deliberate, supported way to run with no SES at all — see that row below. Leaving it unset with `SEND_WORKER_ENABLED` at its default (`true`) still crash-loops exactly as before; the two conditions are both required so an operator who simply forgot this variable is not silently mistaken for one who wants no email. |
...
| `SES_INBOUND_BUCKET` | no | — | S3 bucket SES writes inbound mail to (PRD §6.5 path 3, `#0057`/`#0058`) — production value `opencircuitsf-inbound`, under the key prefix `unsubscribe/`. `internal/inbound.NewS3Store` treats an empty value the same as `SESEventsTopicARN` below: not a boot failure, just nothing to fetch successfully until it's set. As of 2026-09-11 the bucket exists in AWS but this variable is not yet set in production `config.env` — see `docs/email-setup.md`'s "Inbound unsubscribe" runbook. |
| `SES_INBOUND_TOPIC_ARN` | no | *(unset → reject all)* | The SNS topic ARN `POST /api/ses/inbound` (`#0058`) accepts notifications from — the `unsubscribe` receipt rule's own S3 action `TopicArn` (`#0057`). Production value `arn:aws:sns:us-east-1:378152330719:opencircuit-inbound-mail`. Deliberately a **separate** value from `SES_EVENTS_TOPIC_ARN` below, never shared: `internal/handlers` dispatches SES message types by `TopicArn`, so one shared topic would let a bounce/complaint notification and an inbound-mail notification satisfy each other's verifier. Same not-yet-provisioned convention as `SES_EVENTS_TOPIC_ARN`: empty means `sesnotify.Verifier` rejects every inbound message rather than refusing to boot. Present in `.env.example` since `#0058`; not yet added to `docs/configuration.md` until this pass, and not yet set in production `config.env` — both gated on `#0057`'s AWS objects existing, see `docs/email-setup.md`. |
...
| `DEV_ADMIN_LOGIN` | no | `false` | `true` establishes an admin session for `ADMIN_EMAIL` on every request on the Postgres serve path, with no passkey ceremony — the `STORAGE=postgres` counterpart to `STORAGE=json`'s always-on `DevAutoLogin`, closing the gap where the mailing subsystem (interests, subscribers, campaigns, suppressions — `CLAUDE.md` §5) has no dev store to exercise it against (`#0402`). Guarded exactly like `MAILER_NOOP` above: the service refuses to start at all — not just to skip the bypass — when this is `true` and `BASE_URL`'s host is anything but `localhost`/`127.0.0.1` (reuses `mailerNoOpAllowed`'s own host predicate, `cmd/opencircuit/main.go`). Never creates the account it targets — startup fails with a named error (`opencircuit seed`, which only creates the missing row, or `scripts/db-reset.sh`, which drops and rebuilds the whole database) if `ADMIN_EMAIL` has no existing, active row. `scripts/dev.sh --postgres` sets this to `true` automatically; see `docs/dev.md`'s "Running against Postgres" section. Production leaves this unset. |
| `SES_SANDBOX` | no | `true` | No live AWS call anywhere in this codebase can detect sandbox status, so this is a manually-set operational flag: leave it `true` until AWS grants SES production access (`CLAUDE.md` §10 item 2), then flip it to `false`. The admin overview dashboard (`#0061`) warns while this is `true`. Added to `.env.example` and here 2026-09-04 (`#0423`) — it was previously absent from both despite the loader reading it since `#0038`/`#0061`. |
...
| `SEND_WORKER_ENABLED` | no | `true` | Set `false` on a second instance to avoid double-sending. Has no effect when `MAILER_NOOP=true` — the worker never starts regardless (see above). `#0472`: set `false` together with a blank `SES_CONFIGURATION_SET` to turn SES off entirely (`CLAUDE.md` §10 item 2's prescribed recipe, made to actually boot as of this issue). `sesSender` becomes `mailing.UnconfiguredMailer`, which fails every `Send` loudly (`mailing.ErrSESUnconfigured`) instead of sending for real or silently no-opping the way `MAILER_NOOP` does — registration/recovery/etc. still enqueue onto `outbound_queue` and simply retry-then-abandon on its existing backoff, the same outcome already documented above for "a magic link requested before SES exists." `SEND_WORKER_ENABLED=false` with `SES_CONFIGURATION_SET` still set constructs the real SES mailer as usual — this combination is scoped to an actually-unconfigured SES, not to the flag alone. |
